...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      if (to.length > 0) {
        const dateDisp = new Date(created.start_date as any).toISOString().split('T')[0] + (created.end_date && created.end_date !== created.start_date ? ` → ${new Date(created.end_date as any).toISOString().split('T')[0]}` : '')
        ...
        `
        await sendMail({ to, subject, html, replyTo: requesterEmail, text: `Leave request by ${addedByUser} (${requesterEmail || 'N/A'}) ${dateDisp} Type: ${created.leave_type}. Reason: ${String(reason)}. Review: ${link}` })
      }
    } catch {}
    ...
    try {
      if (after) {
        ...
        if (toEmail) {
          ...
          `
          await sendMail({ to: [toEmail], subject, html, text: `Your leave request for ${dates} (${after.leave_type}) updated. ${statusParts.join(', ')}. View: ${link}` })
        }
      }
    } catch {}
