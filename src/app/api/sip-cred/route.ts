...
export async function GET(req: NextRequest) {
  try {
    ...
    const [rows] = await pool.execute(`SELECT email, extension, IF(sip_password IS NULL OR sip_password = '', 0, 1) AS hasPassword FROM ${DB_NAME}.sip_credentials WHERE email = ? LIMIT 1`, [email])
    // @ts-ignore
