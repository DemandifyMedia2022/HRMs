    ...
    try {
      ...
      if (to.length > 0) {
        ...
        await sendMail({
          ...
          html,
          replyTo: me.email,
          text: `New attendance update request by ${me.name} (${me.email}) on ${dateDisp} -> ${desired_status}. Reason: ${String(reason)}. Review: ${link}`
        });
      }
    } catch {}
    ...
    try {
      ...
      if (toEmail) {
        ...
        await sendMail({
          ...
          subject,
          html,
          text: `Your attendance request (${dateDisp}) is ${approval}. ${feedback ? 'Feedback: ' + feedback : ''} Review: ${link}`
        });
      }
    } catch {}
