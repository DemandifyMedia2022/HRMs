...
export async function POST(req: Request) {
  try {
    ...
    if (!/^\d{2}:\d{2}$/.test(start_time) || !/^\d{2}:\d{2}$/.test(end_time)) {
      return NextResponse.json({ success: false, message: "start_time and end_time must be in HH:mm format." }, { status: 400 })
    }
    ...
    return NextResponse.json({ success: true, shift_time: shiftTime, updated, inserted, updated_names: updatedNames, inserted_names: insertedNames, skipped_names: skippedNames })
  } catch (e: any) {
    return NextResponse.json({ success: false, message: e?.message || "Error" }, { status: 500 })
  }
}