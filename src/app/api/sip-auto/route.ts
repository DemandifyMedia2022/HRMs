...
export async function GET(req: NextRequest) {
  try {
    ...
    const [erows] = await pool.execute(`SELECT extension, username, password FROM ${DB_NAME}.extensions WHERE extension = ? LIMIT 1`, [ext])
    // @ts-ignore
    ...
    return NextResponse.json({ extension: rec.extension, password: rec.password || '', username: rec.username || '', assigned: true, found: true })
  } catch (e: any) {
