...
export async function POST(request: NextRequest) {
  try {
    ...
    const user = await (prisma as any).$queryRawUnsafe(
      `SELECT Full_name, emp_code FROM users WHERE id = ?`,
      userId
    )
    ...
    if (existing && existing.length > 0) {
      // Update
      const fields = Object.keys(taxData).map(k => `${k} = ?`).join(", ")
      const values = [...Object.values(taxData), userId]
      await (prisma as any).$queryRawUnsafe(
        `UPDATE Tax SET ${fields} WHERE user_id = ?`,
        ...values
      )
    } else {
      // Insert
      ...
      const placeholders = Object.keys(taxData).map(() => "?").join(", ")
      await (prisma as any).$queryRawUnsafe(
