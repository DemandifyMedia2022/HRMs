      ...
      <div className="p-4 space-y-3 bg-slate-50 dark:bg-slate-950 overflow-x-hidden">
        ...
        <Card className="rounded border border-slate-200 dark:border-slate-800 max-w-[1200px]">
          <CardContent className="py-2 px-3">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              <div>
                <Label htmlFor="search" className="text-[11px] mb-1 block">Search</Label>
                <Input
                  id="search"
                  type="text"
                  value={search}
              ...
              <div>
                <Label htmlFor="month" className="text-[11px] mb-1 block">Month</Label>
                <Input
                  id="month"
                  type="month"
                  value={month}
        ...
        <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded max-w-[1200px] overflow-hidden">
          <div className="border-b border-slate-200 dark:border-slate-800 px-3 lg:px-4 py-2">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Employee Records ({data.length})</h2>
          </div>
          <div className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-260px)] w-full">
            <table className="min-w-[1200px] text-xs">
              <thead className="sticky top-0 z-10">
                <tr className="bg-slate-100 dark:bg-slate-800">
                  <th className="border border-slate-300 dark:border-slate-700 px-2.5 py-2 text-left font-medium text-xs">Sr No</th>
              ...
              <tbody>
                {data.length > 0 && data.map((row, index) => (
                    <tr key={row.id} className={index % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-slate-50 dark:bg-slate-850'}>
                      ...
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 font-medium">{row.emp_code || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 font-medium">{row.Full_name || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.employment_status || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.company_name || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.Business_unit || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.department || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.job_role || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.branch || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.joining_date || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.pan_card_no || ''}</td>
                      ...
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.bank_name || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.bank_name || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.IFSC_code || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.Account_no || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{getCurrentDate()}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{getCurrentDate()}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 font-semibold text-green-600 dark:text-green-400">{row.net_pay || row.netSalary || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.employment_type || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.Full_name || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 text-slate-400">NA</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 text-slate-400">NA</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.contact_no || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.Personal_Email || ''}</td>
                      ...
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.gross_salary || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.Basic_Monthly_Remuneration || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.HRA_Monthly_Remuneration || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.OTHER_ALLOWANCE_Monthly_Remuneration || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.pay_days || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.lop_days || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.basic_earned || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.hra_earned || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.other_earned || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.total_earning || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.PF_Monthly_Contribution || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.Employee_Esic_Monthly || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.income_tax || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.professional_tax || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5">{row.total_deduction || ''}</td>
                      <td className="border border-slate-300 dark:border-slate-600 px-2.5 py-1.5 font-semibold">{row.net_pay || ''}</td>
