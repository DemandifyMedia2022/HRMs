...
export default function LetterGenerationPage() {
  ...
  const remainingCount = letterCategories.reduce(
    (acc, cat) => acc + cat.letters.filter(l => !l.migrated && !l.disabled).length,
    0
  );
      ...
      <div className="flex flex-1 flex-col gap-4 p-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
        ...
        <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Letter Generation</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {letterCategories.map((category, index) => (
            <div
              key={index}
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded"
            >
