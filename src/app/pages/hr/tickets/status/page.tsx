...
export default function RaiseTicketsPage() {
        ...
        if (res.ok) {
          ...
          if (allowedTypes.length > 0) {
            ...
            filteredComplaints = allComplaints.filter(
              (c: Complaint) => c.name?.toLowerCase().trim() === userName.toLowerCase().trim() || 
                              c.added_by_user?.toLowerCase().trim() === userName.toLowerCase().trim()
            );
          }
  ...
  const filteredComplaints = complaints.filter(complaint => {
    ...
    if (filter === 'my-raised') return complaint.name?.toLowerCase().trim() === currentUser.toLowerCase().trim() || 
                                         complaint.added_by_user?.toLowerCase().trim() === currentUser.toLowerCase().trim();
    return true;
  });
      ...
      <div className="p-6">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mb-6 flex gap-2 border-b bg-white px-4 rounded-t-lg">
            ...
            >
              My Raised ({complaints.filter(c => c.name?.toLowerCase().trim() === currentUser.toLowerCase().trim() || 
                                                    c.added_by_user?.toLowerCase().trim() === currentUser.toLowerCase().trim()).length})
            </button>
          </div>
