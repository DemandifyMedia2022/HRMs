...
export default function LabourWelfareDetailsPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      const json = await res.json()
      if (json.success) {
        alert('Labour welfare fund slab saved successfully!')
        fetchSlabs()
      } else {
        alert('Error saving slab: ' + json.error)
      }
    } catch (error) {
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 md:gap-6 md:p-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Labour Welfare Fund</h1>
