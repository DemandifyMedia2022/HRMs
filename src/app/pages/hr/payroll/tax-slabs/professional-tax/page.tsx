  ...
  const handleSave = async () => {
    try {
      ...
      if (json.success) {
        ...
        alert("Professional tax slabs saved successfully!")
        await fetchSlabData() // Wait for refetch to complete
        console.log('Data refetched successfully')
      } else {
        alert("Error saving slabs: " + json.error)
      }
    } catch (error) {
      console.error("Error saving slabs:", error)
      alert("An error occurred while saving the slabs")
    }
  }
            ...
            <div className="flex gap-6">
              <div className="flex items-center space-x-2">
                <input
                  type="radio"
                  id={`all-${slabNumber}`}
                  name={`gender-${slabNumber}`}
                  value="All"
                  checked={formData[`gender${slabSuffix}` as keyof SlabData] === "All"}
                  onChange={(e) => handleInputChange(`gender${slabSuffix}` as keyof SlabData, e.target.value)}
                  className="h-4 w-4 cursor-pointer"
                />
                <Label htmlFor={`all-${slabNumber}`} className="font-normal cursor-pointer">All</Label>
              </div>
              <div className="flex items-center space-x-2">
                <input
                  type="radio"
                  ...
                  name={`gender-${slabNumber}`}
                  value="Male"
                  checked={formData[`gender${slabSuffix}` as keyof SlabData] === "Male"}
                  onChange={(e) => handleInputChange(`gender${slabSuffix}` as keyof SlabData, e.target.value)}
                  className="h-4 w-4 cursor-pointer"
                />
                <Label htmlFor={`male-${slabNumber}`} className="font-normal cursor-pointer">Male</Label>
              </div>
              <div className="flex items-center space-x-2">
                <input
                  type="radio"
                  id={`female-${slabNumber}`}
                  name={`gender-${slabNumber}`}
                  value="Female"
                  checked={formData[`gender${slabSuffix}` as keyof SlabData] === "Female"}
                  onChange={(e) => handleInputChange(`gender${slabSuffix}` as keyof SlabData, e.target.value)}
                  className="h-4 w-4 cursor-pointer"
                />
                <Label htmlFor={`female-${slabNumber}`} className="font-normal cursor-pointer">Female</Label>
              </div>
            </div>
          </div>
        ...
        <div className="flex">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-6 pb-4 border-b-2 border-primary">
              <h1 className="text-xl font-bold text-primary">PROFESSIONAL TAX</h1>
              <Button variant="secondary" onClick={() => router.back()} className="w-24">
                Back
              </Button>
            </div>

            <div className="space-y-6" key={formData.id || 'default'}>
