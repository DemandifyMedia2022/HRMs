...
export default function Page() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const ts = Date.now()
        const res = await fetch(`/api/payroll/professional-tax-slabs?_t=${ts}`, { cache: "no-store", credentials: "include" })
        const j = await res.json()
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              <Button variant="outline" size="sm" asChild className="justify-start gap-2">
                <Link href="/pages/hr/payroll/tax/process-attendance"><IconCalendar className="h-4 w-5" /> Process Attendance</Link>
              </Button>
              <Button variant="outline" size="sm" asChild className="justify-start gap-2">
                <Link href="/pages/hr/payroll/statutory"><IconShieldCheck className="h-4 w-4" /> Statutory</Link>
              </Button>
              <Button variant="outline" size="sm" asChild className="justify-start gap-2">
                <Link href="/pages/hr/payroll/tax-slabs"><IconChartBar className="h-4 w-4" /> Tax Slabs</Link>
              </Button>
              <Button variant="outline" size="sm" asChild className="justify-start gap-2">
                <Link href="/pages/hr/payroll/tax-estimator"><IconCalculator className="h-4 w-4" /> Tax Estimator</Link>
              </Button>
              <Button variant="outline" size="sm" asChild className="justify-start gap-2">
                <Link href="/pages/hr/payroll/tax/update"><IconSettings className="h-4 w-4" /> Tax Structure</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button onClick={() => { setQ(""); fetchRows("") }} variant="outline" size="sm">
                  ...
                  <TableBody>
                    {rows.map((row: TaxRow, idx: number) => (
                      <TableRow key={`${row.user_id ?? row.id ?? row.emp_code ?? idx}`}>
                        <TableCell className="font-medium text-primary">{String(row.user_id ?? row.id ?? "")}</TableCell>
              ...
              <div className="space-y-3">
                {selected && Object.entries(selected).map(([k, v]) => (
                  <div key={k} className="grid grid-cols-2 gap-4 py-3 border-b">
                    <div className="font-semibold text-sm text-muted-foreground capitalize">{k.replace(/_/g, " ")}</div>
