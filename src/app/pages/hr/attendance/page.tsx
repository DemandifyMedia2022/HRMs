                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-7 gap-1.5 text-xs">
                        {headers.map(h => (
                          <div key={h} className="text-center font-medium text-muted-foreground py-1">
                            {h}
                          </div>
                        ))}
                        {cells.map((c, idx) => {
                                ...
                                <div className="space-y-1">
                                  <div className="text-[11px]">
                                    <span
                                      ...
                                      style={{ borderColor: cellBorder }}
                                    >
                                      <span className={`font-medium ${isPresentVariant ? 'text-yellow-600' : base.text}`}>
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                {selected.event.extendedProps.shift_time ? (
                  <div>
                    <span className="text-muted-foreground">Shift:</span> {selected.event.extendedProps.shift_time}
                  </div>
                ) : null}
                {selected.event.extendedProps.in_time && selected.event.extendedProps.in_time !== 'N/A' ? (
                  <div>
                    <span className="text-muted-foreground">In:</span>{' '}
                    {formatTime(selected.event.extendedProps.in_time)}
                  </div>
                ) : null}
                {selected.event.extendedProps.out_time && selected.event.extendedProps.out_time !== 'N/A' ? (
                  <div>
                    <span className="text-muted-foreground">Out:</span>{' '}
                    {formatTime(selected.event.extendedProps.out_time)}
                  </div>
                ) : null}
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {times.map((t, i) => (
                        <span
                          key={i}
                          className="inline-flex items-center rounded-md border px-2 py-1 text-[11px] text-muted-foreground"
                        >
                          {t}
                        </span>
                      ))}
