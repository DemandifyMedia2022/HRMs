...
export default function StatusPage() {
  ...
  const campaigns = useMemo(() => {
    const map = new Map<string, { total: number; pending: number; approved: number; rejected: number; lastDate: string | null }>()
  ...
  return (
    <>
      ...
      <div className="p-6">
        ...
        <p className="text-muted-foreground mb-6">Overview by campaign. Click a campaign to see its records and start the pending queue.</p>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Campaign</th>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Approved</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Rejected</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Last Updated</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {campaigns.map((c) => (
                  <tr key={c.name} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{c.lastDate ? new Date(c.lastDate).toLocaleDateString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
