...
export default function Page() {
  ...
  const toggleUser = (fullName: string) => {
    setSelected(prev => (prev.includes(fullName) ? prev.filter(n => n !== fullName) : [...prev, fullName]));
  };
                ...
                {query ? (
                  <div className="mb-3 border rounded p-2 bg-white max-h-48 overflow-auto">
                    ...
                    {users.filter(u => {
                      ...
                      if (u.Full_name && selected.includes(u.Full_name)) return false;
                      return name.includes(q) || emp.includes(q);
                    }).length === 0 && <div className="text-sm text-gray-500">No matches</div>}
                  </div>
                ) : null}
                ...
                {selected.length > 0 ? (
                  <div className="mb-3 flex flex-wrap gap-2">
                    {selected.map(name => (
                      <span
                        key={name}
                        className="inline-flex items-center gap-2 bg-primary/10 text-primary border border-primary/20 px-2 py-0.5 rounded"
                      >
                        {name}
                        <Button
                          type="button"
