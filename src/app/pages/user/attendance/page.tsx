...
export default function Page() {
    ...
    async function init() {
      try {
        ...
        if (!ignore) {
          ...
          try {
            const u = m?.emp_code ? `/api/essl/trigger?emp_code=${encodeURIComponent(String(m.emp_code))}` : `/api/essl/trigger`;
  ...
  const selectedUser = useMemo(() => {
    ...
    return data.find((u) => u.employeeName?.toLowerCase() === q) ||
           data.find((u) => u.employeeName?.toLowerCase().includes(q));
  }, [data, meName]);
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              >Prev</Button>
              <div className="text-sm">{months[month]} {year}</div>
              <Button
                variant="outline"
                onClick={() => {
                  setMonth((m) => {
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-sm text-gray-600">Your attendance is not available for {year}. If this persists, contact HR.</div>
          ) : (
            <>
          ...
          {selected.holidayName ? (
            <div className="space-y-3 text-sm">
              ...
              {selected.holidayStart || selected.holidayEnd ? (
                <div className="text-gray-600">{selected.holidayStart || ''} {selected.holidayEnd ? `→ ${selected.holidayEnd}` : ''}</div>
              ) : null}
            </div>
          ) : selected.leaveType ? (
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                {selected.event.extendedProps.shift_time ? (
                  <div><span className="text-gray-600">Shift:</span> {selected.event.extendedProps.shift_time}</div>
                ) : null}
                {selected.event.extendedProps.in_time && selected.event.extendedProps.in_time !== 'N/A' ? (
                  <div><span className="text-gray-600">In:</span> {formatTime(selected.event.extendedProps.in_time)}</div>
                ) : null}
                {selected.event.extendedProps.out_time && selected.event.extendedProps.out_time !== 'N/A' ? (
                  <div><span className="text-gray-600">Out:</span> {formatTime(selected.event.extendedProps.out_time)}</div>
                ) : null}
                <div><span className="text-gray-600">Login:</span> {formatDuration(selected.event.extendedProps.login_hours)}</div>
                <div><span className="text-gray-600">Break:</span> {formatDuration(selected.event.extendedProps.break_hours)}</div>
                <div><span className="text-gray-600">Total:</span> {formatDuration(selected.event.extendedProps.total_hours)}</div>
              </div>
              {(() => {
                ...
                return times.length > 0 ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {times.map((t, i) => (
                        <span key={i} className="inline-flex items-center rounded-md border px-2 py-1 text-[11px] text-gray-600">
                          {t}
                        </span>
                      ))}
                    </div>
