    ...
    <>
      ...
      <div className="p-6">
        ...
        <p className="text-muted-foreground mb-6">
          Fill fields and submit to store in <code>dm_form</code>.
        </p>

        {/* Auth gate */}
        {authLoading && <div className="text-sm text-gray-600 mb-4">Checking permissions...</div>}
              ...
              <div key={f} className="space-y-1.5">
                ...
                {f === 'f_conatct_no' ? (
                  ...
                  <input
                    ...
                    value={formData[f] ?? ''}
                    onChange={e => onChange(f, e.target.value)}
                    type={f.includes('date') ? 'datetime-local' : f.includes('email') ? 'email' : 'text'}
                  />
                )}
              </div>
            ))}
