...
export default function RaiseTicketsPage() {
      ...
      if (userRes.ok) {
        ...
        if (res.ok) {
          ...
          if (allowedTypes.length > 0) {
            ...
            filteredComplaints = allComplaints.filter((c: Complaint) => 
              allowedTypes.includes(c.issuse_type)
            )
          } else {
            // Regular user - show only their own complaints
            setIsDepartmentUser(false)
            filteredComplaints = allComplaints.filter((c: Complaint) => 
              c.name === userName || c.added_by_user === userName
            )
          }
  ...
  async function handleAcknowledge(complaintId: number) {
    ...
    try {
      ...
      if (res.ok) {
        alert("Ticket acknowledged successfully! User has been notified via email.")
        fetchUserAndComplaints()
      } else {
  ...
  const filteredComplaints = complaints.filter((complaint) => {
    ...
    if (filter === "acknowledged") return complaint.acknowledgement_status?.toLowerCase() === "acknowledged" && complaint.status?.toLowerCase() !== "resolved"
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          Pending ({complaints.filter((c) => c.status?.toLowerCase() === "pending" && !c.acknowledgement_status).length})
        </button>
        <button
        ...
        >
          Acknowledged ({complaints.filter((c) => c.acknowledgement_status?.toLowerCase() === "acknowledged" && c.status?.toLowerCase() !== "resolved").length})
        </button>
        <button
            ...
            >
              ...
              <div className={viewMode === "list" ? "flex-shrink-0 w-64" : "w-full"}>
                ...
                <div className="mb-3">
                  <div className="flex justify-between items-start mb-2">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        getStatusColor(complaint.status, complaint.acknowledgement_status)
                      }`}
                    >
                  ...
                  <h3 className="text-base font-bold text-gray-900 mb-1">
                    {complaint.issuse_type}
                  </h3>
                  <p className="text-xs text-gray-500">
                    {new Date(complaint.raisedate).toLocaleDateString()}
                  </p>
                </div>
              ...
              <div className={`flex flex-col ${viewMode === "list" ? "flex-1" : "w-full"}`}>
                ...
                <div className={`mb-4 ${viewMode === "list" ? "flex-1" : "flex-grow"}`}>
                  ...
                  <p className={`text-sm text-gray-800 ${viewMode === "grid" ? "line-clamp-3" : ""}`}>{complaint.reason}</p>
                </div>
                    ...
                    selectedComplaint?.id === complaint.id ? (
                      <div className="space-y-2">
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 mb-2">
                          <p className="text-xs font-semibold text-blue-900">✓ Acknowledged by {complaint.acknowledgement_by}</p>
                        </div>
                        <textarea
